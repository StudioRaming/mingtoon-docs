...
BOOTH インストーラーの更新ウィンドウには**プレビュー版を含める**チェックボックスがあります。
既定はオフで、オンにしたプロジェクトだけが次の安定版より先に配信される試験版を確認します。
プレビュー版をインストールした後でチェックを外すと、インストール済みプレリリースより番号が低くても、安定版チャンネルの最新版を案内します。
更新が途中で終わり、レシートが旧バージョンのままでも、インストール済みパッケージが検証済み一覧から再構築します。
この修復のためにbootstrap DLLは変更しません。
