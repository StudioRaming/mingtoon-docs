...
- 2Dリムライト · 2Dシャドウ · 内部2D境界は **ホストによるカメラ深度の提供に依存** します。VRChatの通常のプレイヤー画面では、アバター側から深度を強制することはできません。
- lilToon変換は **相互運用ツール** であり、他のシェーダーの結果を数学的に複製するものではありません。
- VCCで導入するBRPコアはソースパッケージで、BOOTHインストーラーは別のDLL配布経路です。どちらも同じBRPコアバージョンを指します。
