...
| **SSAO Radius** | How far the occlusion search reaches, in metres (0.001-0.25 is 1mm-25cm). It is world size rather than screen pixels, so the thickness holds as the camera pulls back or the field of view changes - but only while `SSAO Distance Compensation` is 1, its default. Lower it and the radius shrinks inside one metre. With the camera very close the on-screen disc meets an internal ceiling and stops growing; that ceiling protects frame time and is not an authored control. `SSAO Fade Range` is a multiple of this value, so changing the radius carries the reach with it. | `_MingSsaoRadius` |
...
| **SSAO Bias** | Slack that keeps depth-buffer rounding error from leaking in as occlusion. It scales with distance by itself, so it only has to be tuned once for every camera range. Raise it slightly if a flat surface still goes blotchy; lower it when shallow creases disappear. A tilted surface no longer occludes itself at all, so a far smaller value is enough than this control used to need. | `_MingSsaoBias` |
| **SSAO Fade Range** | How close an occluder has to be to count, given as a MULTIPLE of `SSAO Radius`. At 1 the reach is one radius, at 2 it is two, and past that the weight falls to zero, so a distant wall does not darken the whole silhouette. Being a multiple rather than metres, it keeps its proportion when you change the radius. | `_MingSsaoFadeRange` |
| **SSAO Far Distance** | The distance, in metres, at which the effect is fully gone. The fade begins at 75% of it, so 12 starts thinning at 9m and reaches zero at 12m. The per-pixel sample count falls over that same stretch, so a character past this distance costs nothing at all - which makes lowering this the surest optimisation in a crowded scene. 0 disables the distance fade, and the saving with it. | `_MingSsaoFarDistance` |
...
| **SSAO Tint** | A color multiplied into the occluded areas only. White tints nothing and leaves the shadow color the material already resolved exactly as it was. | `_MingSsaoColor` |
...
| **SSAO Saturation** | Saturation of the occluded areas. 1 leaves them alone, 0 drains them to gray, and above 1 the color deepens there and nowhere else. | `_MingSsaoSaturation` |
| **Visibility in Cast Shadow** | 0 hides SSAO inside the cast shadow; 1 keeps it fully visible (default). Lower it to stop occlusion from double-darkening an area a shadow already owns. | `_MingSsaoCastShadowVisibility` |
| **Visibility in 2D Shadow** | 0 hides SSAO inside the 2D shadow; 1 keeps it fully visible (default). Lower it when occlusion shows through the drawn 2D shape. | `_MingSsao2DShadowVisibility` |
...
| **Apply to Inner Outline** | Also applies the painted pressure to the Inner 2D Edge width. This toggle and Pressure Source live in the Classic Hull group, but this one keeps affecting the inner edge even with the hull turned off - on a mesh with black vertex colors it makes the inner edge disappear completely. | `_OutlineInnerEdgePressureApply` |
