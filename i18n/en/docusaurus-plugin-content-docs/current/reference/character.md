...
| **Proxy Axis** | Length direction in object space. Default (0,1,0) points head-up. Used by Cylinder and Capsule; avoid a zero vector. | `_FaceProxyAxisOS` |
...
| **Face Proxy Depth (Experimental)** | Writes the proxy volume into the depth buffer and the shadow map in place of the face mesh. The colour pass is untouched, so a fringe still casts a crisp shadow shaped by the real face. It does nothing on a material whose proxy has never been placed. | `_MingFaceProxyDepthEnabled` |
| **Proxy Depth Amount** | Blend between the real face position and the proxy surface. At 1 the substitution is complete; at 0 nothing moves. This is the dial that decides how much of the nose and brow survive. | `_MingFaceProxyDepthAmount` |
| **Proxy Depth Scope** | CameraDepthOnly (0, default) substitutes only in the camera depth render, so world AO and the depth modules see a smooth head. CameraDepthAndShadow (1) also replaces the real-time shadow caster with a shrunken proxy, which removes face self-shadowing. The shadow the face casts on others becomes a dome silhouette, so adopt the second step deliberately. | `_MingFaceProxyDepthMode` |
| **Shadow Caster Shrink (m)** | How far, in metres, the shadow-casting proxy is inset along its radius. The inset is what puts every real surface point outside the caster, so the face cannot be caught in its own shadow. Too small and concave areas such as the eye sockets stay inside the dome and shadow again; too large and the shadow the jaw throws onto the neck thins out. Used only while Proxy Depth Scope is CameraDepthAndShadow. | `_MingFaceProxyDepthShadowShrink` |
...
| **Normal Press Amount** | How strongly the face normal override is applied. At 0 the Normal Method, proxy sphere center and proxy sphere radius are all ignored and the raw mesh normal is used. On a baked material this value still sets the amount live. At 0 you get the original normal. | `_FaceNormalFlattenAmount` |
