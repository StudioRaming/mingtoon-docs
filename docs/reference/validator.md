...
**오류.** 셰이더가 정규화 영역 조절 컨트롤을 전부 `Range(0, 1)`로 선언하지 않았습니다. 어긋난 프로퍼티 이름이 보고에 나옵니다.

Fresnel Width / Softness, Front Light Size / Softness / 방향 비율 / 코어 크기 같은 영역 조절 프로퍼티는 0~1 슬라이더 규약입니다. Range 타입이 아니거나 범위가 0~1이 아니면 **영역 UI가 0 = 없음, 1 = 최대, 그리고 정규화된 부드러움·방향 비율을 더 이상 보장하지 못합니다.**

**조치** — 릴리스 패키지에서 `Assets/StudioRaming/MingToon/Shaders`를 복구한 뒤 다시 검증하세요.
...
**오류.** 투명 큐에서 카메라 깊이 효과를 씁니다. 정렬과 자기 깊이가 불안정해 **2D 림라이트와 2D 그림자가 깜빡이거나 뒤에 있는 표면을 읽습니다.**
...
**오류.** URP 재질이 2D 림라이트나 2D 그림자를 쓰는데 필요한 Renderer Feature가 없어 **두 효과가 아무것도 그리지 않습니다.** 세 가지 경우가 있습니다.
...
**경고.** Bake Manifest가 만들어진 셰이더 생성기 버전이 현재 MingToon 패키지가 생성하는 버전보다 낮습니다. 보고에 Manifest 경로와 양쪽 버전이 나옵니다.

Manifest에 기록된 baked 재질은 **여전히 렌더링되지만 예전 생성기가 쓴 셰이더로 그려집니다.** 지금 다시 구우면 들어갈 수정이 하나도 반영돼 있지 않고, **빌드 준비 검사가 이 재질들을 거부합니다.**

**조치** — 밍툰 매니저에서 베이크를 다시 실행해 재생성하세요. → [수동 Bake와 복원](/workflow/bake-and-restore)
...
**오류.** Built-in RP 깊이 공급자를 찾지 못했거나 형태가 바뀌었습니다. 어떤 카메라도 깊이 텍스처를 요청받지 않으므로 **2D 림라이트 · 2D 그림자 · 깊이 기반 아웃라인이 설정과 무관하게 비어 있습니다.**
