...
1. 아바타 루트에 MLC 컴포넌트를 추가합니다. 아래 메뉴로도 됩니다.
...
4. **예산·검증** 에서 합계와 생성 가능 여부를 확인합니다. 빨간 오류를 먼저 해결합니다.
5. Play Mode에서 메뉴가 열리고 값이 바뀌는지 확인한 뒤 업로드합니다.

예산 화면에 빨간 오류가 없고 Play Mode에서 메뉴가 열리면 성공입니다.
