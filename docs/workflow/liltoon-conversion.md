...
Face / Skin / Common 역할, 변환 룩, 출력 경로, 변환 되돌리기와 손실 보고서를 한곳에서 처리합니다. → [밍툰 매니저](/workflow/character-manager#1--변환)
:::
...
밍툰 매니저의 `변환 되돌리기 (MingToon 이전 재질로)`는 기록된 원본 GUID로 현재 슬롯을 한 Undo 단계에서 되돌립니다. 생성된 변환 재질과 메시 bake 에셋은 자동 삭제하지 않습니다.
