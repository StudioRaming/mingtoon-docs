...
2D 림 · 2D 그림자 · Inner Edge는 카메라 깊이 텍스처를 읽습니다.
...
1. 모드를 로드한 뒤 2D 효과가 보이는지 봅니다.
2. 안 보이면 [Camera Depth 진단 순서](/guides/camera-depth#안-보일-때-진단-순서)를 밟습니다.
3. 깊이를 확보할 수 없는 구성이라면, 깊이가 필요 없는 **Hull 외곽선**과 **림 라이트**로 실루엣을 대신 잡습니다.
...
- 씬 조명을 통제할 수 없는 연출이라면 `베이스 색 유지`와 `최소 밝기`를 올려 두세요. → [그림자 잡기](/guides/shadow#어두운-씬에서-검게-뭉칠-때)
