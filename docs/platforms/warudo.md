...
2D 림라이트·2D 그림자·이너 아웃라인·SSAO 같은 화면 깊이 효과에는 Warudo 카메라의 깊이 텍스처가 필요합니다. 0.1.10 패키지는 이를 공급하는 독립 Warudo 플러그인 소스를 포함합니다.
...
- Built-in의 포인트·스폿 추가광은 ForwardAdd 패스로 들어옵니다. 재질의 `추가 광원 수신량`, `추가 광원 강도`, 툰 경계 설정으로 조정합니다.
...
- 메인 화면과 실제 송출 카메라에서 2D 림라이트·2D 그림자·내부 경계를 확인했습니다.
