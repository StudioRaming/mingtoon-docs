...
MingToon `0.1.10` 공개 오픈 베타 기준입니다. BRP 본체의 현재 배포 범위를 설명합니다. URP는 이번 BRP 오픈 베타에 포함되지 않으며, 모든 상업 라이선스에 포함됩니다.
...
- URP는 현재 BRP 오픈 베타에 포함되지 않으며, 모든 상업 라이선스에 포함됩니다. 이 문서는 현재 BRP 지원 범위를 설명합니다.
...
- VCC로 설치하는 BRP 코어는 소스 패키지이며, BOOTH 설치기는 별도의 DLL 배포 경로입니다. 두 경로는 같은 BRP 코어 버전을 가리킵니다.
