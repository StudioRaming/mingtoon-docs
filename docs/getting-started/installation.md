...
0.1.5 이하로 작업하던 프로젝트를 0.1.6으로 올렸다면, 아래 두 가지를 **한 번씩** 해 두어야 합니다.
...
생성 셰이더 캐시 버전이 **28에서 29로** 올라가 기존 베이크 산출물이 무효가 됩니다. 다음 빌드/업로드에서 자동으로 다시 구워지므로 손으로 지울 것은 없지만, 그 한 번은 시간이 더 걸립니다.
