...
MingToon 0.1.10 오픈 베타 본체는 BRP 대상입니다. URP는 이번 BRP 오픈 베타에 포함되지 않으며, 모든 상업 라이선스에 포함됩니다.
...
머티리얼 스키마가 **10에서 11로** 올라갔습니다. `림 라이트 마스크 사용` · `림 그림자 마스크 사용` 두 토글이 새로 생겼는데, 이전 재질에는 이 토글이 없어 **꺼진 상태로 읽힙니다.** 마이그레이션이 흰색 기본값이 아닌 마스크를 찾아 토글을 켜 줍니다.
