...
BOOTH 설치기의 업데이트 창에는 **프리뷰 버전 포함** 체크박스가 있습니다.
기본은 꺼짐이며, 켠 프로젝트만 다음 정식 버전보다 먼저 나오는 시험판을 확인합니다.
프리뷰 설치를 마친 뒤 체크를 끄면, 설치된 시험판보다 번호가 낮아도 정식 채널의 최신 버전을 안내합니다.
업데이트가 중간에 끝나 설치 영수증이 이전 버전에 남아도, 설치된 패키지가 검증된 설치 목록으로 영수증을 다시 만듭니다.
bootstrap DLL은 이 복구를 위해 바뀌지 않습니다.
