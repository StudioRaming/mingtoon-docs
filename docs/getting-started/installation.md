...
:::caution VRChat 대상에서 나오는 Unity 버전 오류는 무시하세요
현재 빌드의 검증기는 `MING-ENV-UNITY-VERSION` 규칙을 **2021.3.x 기준으로만** 판정합니다. VRChat에 필요한 2022.3.22f1에서는 이 항목이 오류로 표시되지만, **VRChat 대상에서는 2022.3.22f1이 올바른 버전입니다.** 다른 검증 항목은 정상적으로 신뢰할 수 있습니다.

이 불일치는 알려진 문제로 수정 예정입니다.
:::
